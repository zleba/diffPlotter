...
double getBestChi2(TString anal, TString var, vector<double> thVec)
{
    ...
    ifstream matFile;
    matFile.open(TString::Format("/home/radek/Dropbox/daniel/DDJets/data/Measurement/fullErrors/data/%s.txt", anal.Data()));
    ...
    double nBest = getNorm(errMat, data, th);
...
void readVFPS(ifstream &matFile, arma::mat &errMat, arma::vec &data)
{
    ...
    //Add diagonal
    for(int i = 0; i < corStat.n_rows; ++i) {
...
void readFPS(ifstream &matFile,  arma::mat &errMat, arma::vec &data)
{
    ...
    while (matFile.is_open() && getline (matFile,line) ) {
        ...
        for(int i = 0; i < shiftMat.n_cols; ++i) { //read all shifts
    ...
    for(int i = 0; i < shiftMat.n_cols; ++i) {
    ...
    //calc off-diagonal -- erij / sqrt(erii * erjj)
    for(int i = 0;   i < corStat.n_rows; ++i) 
    for(int j = i+1; j < corStat.n_cols; ++j) {
        corStat(i,j) = corStat(i,j) * errStat(i) * errStat(j);
        corStat(j,i) = corStat(i,j);
    }
...
void readLRG(ifstream &matFile, arma::mat &errMat, arma::vec &data)
{
    ...
    bool hasCorr = false;
    while (matFile.is_open() && getline (matFile,line) ) {
        if(TString(line).BeginsWith("corr")) {
            hasCorr = true;
            break;
        }
